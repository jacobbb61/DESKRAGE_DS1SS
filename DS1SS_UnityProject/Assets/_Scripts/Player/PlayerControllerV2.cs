...
public class PlayerControllerV2 : MonoBehaviour
{
   ...
   // private GameObject
    ...
    private void Start()
    {
        ...
        GameObject.FindGameObjectWithTag("Listener").gameObject.transform.parent = transform;
        ...
        if (nightmare) 
        {
            if (CurrentEstus > 6) { CurrentEstus = 6; }
            if (MaxEstus > 6) { MaxEstus = 6; }
        }
        else
        {
    ...
    {
        int layerMask = ~(LayerMask.GetMask("Player"));
        RaycastHit2D hit = Physics2D.Raycast(transform.position, new Vector2(0, -1), HeavyAttackFollowUpRange, mask);
    ...
    public void GiveOneEstus()
    {
        Debug.Log("add estus 1");
        if (MaxEstus < 6)
        {
            MaxEstus++; 
            if (CurrentEstus < 6)
