...
public class Pursuer : MonoBehaviour
{
    ...
    public GameObject Nightmare_Pos;
    public GameObject Nightmare_ResetPos_1;
    public GameObject Nightmare_ResetPos_2;
    public bool NightmareSwap;
        ...
        else
        {
            ...
            switch (Behaviour)
            {
                ...
                case "Hostile":
                    ...
                    if (!IsCoolingDown)
                    { 
                        if (CanPhaseChanged == true) 
                        {
                            if (nightmare)
                            {
                                Nightmare_Attack_Start();
                            }
                            else
                            {
    ...
    public void TakeHeavyDamage()
    {
        ...
        AddDamage(10);
        if (Health <= 0) { StopAllCoroutines(); StartCoroutine(Death()); arenaManager.Victory(); }
        if (Health <= MaxHealth / 2 && !arenaManager.IsSecondPhase) { CanPhaseChanged = true; }
    }
    ...
    public void Nightmare_Attack_Start()
    {
        CanPhaseChanged = false;
        Behaviour = "Attacking";
        IsCoolingDown = false;

        Anim.Play("ThePursuerAnim_Dissapear");
        Speed = 5f;
    }

    public void Nightmare_Attack_Deploy()
    {
        gameObject.transform.position = Nightmare_Pos.transform.position; 
        Anim.Play("ThePursuerAnim_Nightmare");
        GetComponent<Rigidbody2D>().constraints = RigidbodyConstraints2D.FreezePosition;
    }
    public void Nightmare_Attack_Reset()
    {
        if (NightmareSwap)
        {
            gameObject.transform.position = Nightmare_ResetPos_1.transform.position;
            NightmareSwap = false;
        }
        else
        {
            gameObject.transform.position = Nightmare_ResetPos_2.transform.position;
            NightmareSwap = true;
        }
        ...
        Anim.Play("ThePursuerAnim_Reapear");

        InstantTurn();
    }
    public void Nightmare_Attack_Finish()
    {
        Behaviour = "Hostile";
        IsCoolingDown = false;
        Anim.Play("ThePursuerAnim_Idle");
    ...
    public void Nightmare_Attack_Spawn()
    {
        nightmareAttackSpawner.spawn();
        arenaManager.SecondPhase();
    }
}
